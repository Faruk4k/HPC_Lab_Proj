...
from prefetchers.ampm.ampm_configs import make_ampm_prefetcher
...
def get_prefetchers(prefetcher_name, pf_level, ampm_config):
    l1d_pf = None
    l2_pf = None
    ...
    if prefetcher_name == "ampm":
        pf_cls = make_ampm_prefetcher(ampm_config)
    else:
...
parser.add_argument(
    "--ampm-config",
    default="deg4",
    help="ampm config name from ampm_configs.py",
)
...
print(f"ampm config : {args.ampm_config}")
print("================================")
