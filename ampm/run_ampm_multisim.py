...
from ampm_configs import make_ampm_prefetcher, ampm_configurations
...
# ../../gem5/build/ALL/gem5.opt --outdir results_ampm_multisim -m gem5.utils.multisim run_ampm_multism.py 
BENCHMARKS = {
    "simple_triad": {
...
def make_board(memory_name, pf_level, ampm_config_name):
    ampmPF = make_ampm_prefetcher(ampm_config_name)
...
def add_ampm_simulation(benchmark_name, benchmark_info, memory_name, pf_level, ampm_config_name):
    ...
    board = make_board(
        ...
        pf_level=pf_level,
        ampm_config_name=ampm_config_name,
    )
    ...
    sim_id = f"{benchmark_name}_ampm_{ampm_config_name}_{pf_level}_{memory_name}"
...
for benchmark_name, benchmark_info in BENCHMARKS.items():
    for memory_name in MEMORIES:
        for pf_level in PF_LEVELS:
            for ampm_config_name in ampm_configurations:
                add_ampm_simulation(
