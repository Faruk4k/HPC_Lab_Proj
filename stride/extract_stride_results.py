...
for run_name in sorted(os.listdir(ROOT)):
    ...
    if "_stride_" not in run_name:
        continue

    benchmark, rest = run_name.split("_stride_", 1)
...
if not rows:
    raise RuntimeError("No Stride rows extracted")
