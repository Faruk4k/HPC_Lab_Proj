...
from imp_configs import make_imp_prefetcher, imp_configurations
...
def make_board(memory_name, pf_level, imp_config_name):
    IMPPF = make_imp_prefetcher(imp_config_name)
...
def add_imp_simulation(benchmark_name, benchmark_info, memory_name, pf_level, imp_config_name):
    ...
    sim_id = f"{benchmark_name}_imp_{imp_config_name}_{pf_level}_{memory_name}"
...
for benchmark_name, benchmark_info in BENCHMARKS.items():
    for memory_name in MEMORIES:
        for pf_level in PF_LEVELS:
            for imp_config_name in imp_configurations:
                add_imp_simulation(
