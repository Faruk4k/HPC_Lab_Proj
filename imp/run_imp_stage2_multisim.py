...
from imp_stage2_configs import make_imp_stage2_prefetcher, imp_stage2_configurations

# Start with 4 or 8. Increase later if stable.
...
def make_board(memory_name, pf_level, imp_config_name):
    IMPPF = make_imp_stage2_prefetcher(imp_config_name)
...
def add_imp_simulation(benchmark_name, benchmark_info, memory_name, pf_level, imp_config_name):
    ...
    board = make_board(
        ...
        pf_level=pf_level,
        imp_config_name=imp_config_name,
    )
    ...
    sim_id = f"{benchmark_name}_imp_stage2_{imp_config_name}_{pf_level}_{memory_name}"
...
for benchmark_name, benchmark_info in BENCHMARKS.items():
    for memory_name in MEMORIES:
        for pf_level in PF_LEVELS:
            for imp_config_name in imp_stage2_configurations:
                add_imp_simulation(
