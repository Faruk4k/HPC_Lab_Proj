...
NEXTLINE = "nextline_results_vs_nopf.csv"
NOPF = "../baseline/nopf_results.csv"

OUTDIR = "plots_nextline_vs_nopf_by_case"
TOP_N = 25
...
nextline["label"] = nextline["config"].apply(lambda x: f"next-line: {x}")
...
def force_include_reference_rows(top, full):
    refs = full[full["label"] == "No prefetch baseline"]
...
def plot_horizontal_bar(sub, title, filename, top_only=True):
    ...
    handles = [
        ...
        plt.Rectangle((0, 0), 1, 1, color="C0", label="Next-line config"),
    ]
...
for benchmark in benchmarks:
    for pf_level in pf_levels:
        for memory in memories:
            sub = nextline[
                (nextline["benchmark"] == benchmark)
                & (nextline["pf_level"] == pf_level)
                & (nextline["memory"] == memory)
            ].copy()
            ...
            case_csv = os.path.join(
                OUTDIR,
                f"{benchmark}_{pf_level}_{memory}_nextline_all_configs.csv",
            )
            ...
            title = f"{benchmark}: Next-line speedup vs no prefetch ({pf_level.upper()}, {memory})"

            filename = os.path.join(
                OUTDIR,
                f"{benchmark}_{pf_level}_{memory}_top{TOP_N}_nextline_vs_nopf.png",
            )

            plot_horizontal_bar(sub, title, filename, top_only=True)

            title = f"{benchmark}: all Next-line configs vs no prefetch ({pf_level.upper()}, {memory})"

            filename = os.path.join(
                OUTDIR,
                f"{benchmark}_{pf_level}_{memory}_all_nextline_vs_nopf.png",
            )
...
winners.to_csv("nextline_winners_vs_nopf_per_case_from_plots.csv", index=False)

print(f"Wrote plots to: {OUTDIR}")
print("Wrote winners CSV: nextline_winners_vs_nopf_per_case_from_plots.csv")
