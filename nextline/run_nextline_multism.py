...
from nextline_configs import make_nextline_prefetcher, nextline_configurations
...
def make_board(memory_name, pf_level, nextline_config_name):
    nextlinePF = make_nextline_prefetcher(nextline_config_name)
...
def add_nextline_simulation(benchmark_name, benchmark_info, memory_name, pf_level, nextline_config_name):
    ...
    board = make_board(
        ...
        pf_level=pf_level,
        nextline_config_name=nextline_config_name,
    )
    ...
    sim_id = f"{benchmark_name}_nextline_{nextline_config_name}_{pf_level}_{memory_name}"
...
for benchmark_name, benchmark_info in BENCHMARKS.items():
    for memory_name in MEMORIES:
        for pf_level in PF_LEVELS:
            for nextline_config_name in nextline_configurations:
                add_nextline_simulation(
