...
class OutOfOrderCPUCore(ArmO3CPU):
    def __init__(self, width, rob_size, num_int_regs, num_fpvec_regs, lsq_size, sve_vl, core_id):
        """
...
class OutOfOrderCPUStdCore(BaseCPUCore):
    def __init__(self, width, rob_size, num_int_regs, num_fpvec_regs, lsq_size, sve_vl, core_id):
        ...
        register file.
        """
        core = OutOfOrderCPUCore(width, rob_size, num_int_regs, num_fpvec_regs, lsq_size, sve_vl, core_id)
        super().__init__(core, ISA.ARM)
        core.isa[0].sve_vl_se = sve_vl


class MyOutOfOrderCPU(BaseCPUProcessor):
    def __init__(self, width, rob_size, num_int_regs, num_fpvec_regs, lsq_size, sve_vl, num_cores):
        """
        ...
        """
        cores = [
            OutOfOrderCPUStdCore(width, rob_size, num_int_regs, num_fpvec_regs, lsq_size, sve_vl, i)
...
class MyO3Processor(SimpleProcessor):
    def __init__(self, width, rob_size, num_int_regs, num_fp_regs, \
                 num_cores: int, sve_vl: int):
        super().__init__(cpu_type=CPUTypes.O3, num_cores=num_cores, isa=ISA.ARM)

        for cores in self.cores:
            corecfg    = cores.core

            corecfg.fetchWidth  = width
            corecfg.decodeWidth = width
            corecfg.renameWidth = width
            corecfg.issueWidth  = width
            corecfg.wbWidth     = width
            corecfg.commitWidth = width

            corecfg.numROBEntries   = rob_size
            corecfg.numPhysIntRegs  = num_int_regs
            corecfg.numPhysFloatRegs = num_fp_regs

            corecfg.branchPred = BiModeBP()

            corecfg.LQEntries = 128
            corecfg.SQEntries = 128
            corecfg.isa[0].sve_vl_se = sve_vl
